...
class ResultScreen:
    ...
    def mark_scenario_completed(self):
        """Marca o cenário como completo no session state"""
        # Pega a etapa atual (budget, authority, need, timeline)
        etapa = self.scenario.data.get('etapa', 'budget')
        
        # Extrai apenas o nome (marcia, paula, carla) do scenario.key
        # scenario.key pode ser "budget_marcia" ou só "marcia"
        scenario_name = self.scenario.key.split('_')[-1]  # Pega a última parte
        
        # Marca como completo com formato correto: etapa_nome_completed
        key = f"{etapa}_{scenario_name}_completed"
